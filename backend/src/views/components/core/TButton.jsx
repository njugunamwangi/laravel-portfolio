...
export default function TButton({
    ...
    if (link) {
        ...
        switch (color) {
            case "indigo":
                classes = [
                    ...classes, "text-indigo-500", "focus:border-indigo-500"
                ];
                break;
            case "red":
                classes = [
                    ...classes, "text-red-500", "focus:border-red-500"
                ];
                break;
        }
    } else {
    ...
    if (circle) {
        classes = [
            ...classes, "h-8", "w-8", "items-center", "justify-center", "rounded-full", "text-sm"
        ];
    } else {
        classes = [
            ...classes, "p-0", "py-2", "px-4", "rounded-md"
        ];
    }
    ...
    return (
        <>
            ...
            {!to && !href && (<button onClick={onClick} to={to} className={classes.join(" ")}>{children}</button>)}
        </>
    )
}
